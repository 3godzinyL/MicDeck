...
uint32_t MdAtomicLoad32(const MD_ATOMIC_U32& v) noexcept {
#ifdef _KERNEL_MODE
...
void MdAtomicStore32(MD_ATOMIC_U32& t, uint32_t v) noexcept {
#ifdef _KERNEL_MODE
...
uint32_t MdAtomicIncrement32(MD_ATOMIC_U32& t) noexcept {
#ifdef _KERNEL_MODE
    return static_cast<uint32_t>(InterlockedIncrement(&t));
#else
    return t.value.fetch_add(1, std::memory_order_acq_rel) + 1u;
#endif
}
uint64_t MdAtomicLoad64(const MD_ATOMIC_U64& v) noexcept {
#ifdef _KERNEL_MODE
...
void MdAtomicStore64(MD_ATOMIC_U64& t, uint64_t v) noexcept {
#ifdef _KERNEL_MODE
...
uint64_t MdAtomicAdd64(MD_ATOMIC_U64& t, uint64_t d) noexcept {
#ifdef _KERNEL_MODE
...
namespace {
uint32_t MdAtomicMax32(MD_ATOMIC_U32& t, uint32_t c) noexcept {
#ifdef _KERNEL_MODE
    LONG o = InterlockedCompareExchange(&t, 0, 0);
    while (static_cast<uint32_t>(o) < c) {
        const LONG r = InterlockedCompareExchange(&t, static_cast<LONG>(c), o);
        if (r == o) return c;
        o = r;
    }
    ...
    while (o < c && !t.value.compare_exchange_weak(
        o, c, std::memory_order_release, std::memory_order_relaxed)) {}
    return o < c ? c : o;
#endif
}
uint32_t MdAtomicMin32(MD_ATOMIC_U32& t, uint32_t c) noexcept {
#ifdef _KERNEL_MODE
    LONG o = InterlockedCompareExchange(&t, 0, 0);
    while (static_cast<uint32_t>(o) > c) {
        const LONG r = InterlockedCompareExchange(&t, static_cast<LONG>(c), o);
...
bool MdStereoRing::Initialize(float* storage, uint32_t capacity) noexcept {
    ...
    // Initialize runs before any stream exists, so storing the cursors directly is safe
    // here and only here.
    storage_ = storage;
    capacity_ = capacity;
    mask_ = capacity - 1u;
    memset(storage_, 0, sizeof(float) * capacity_ * MD_INTERNAL_CHANNELS);
    MdAtomicStore64(write_frame_, 0);
    MdAtomicStore64(read_frame_, 0);
    MdAtomicStore64(dropped_frames_, 0);
    MdAtomicStore64(silent_frames_, 0);
    MdAtomicStore64(discarded_frames_, 0);
    MdAtomicStore32(high_watermark_, 0);
    MdAtomicStore32(low_watermark_, capacity_);
    MdAtomicStore32(generation_, 0);
    MdAtomicStore32(flush_request_, 0);
    flush_acked_ = 0;
    return true;
}

void MdStereoRing::Detach() noexcept {
    storage_ = nullptr;
    capacity_ = 0;
    mask_ = 0;
}

void MdStereoRing::RequestFlush() noexcept {
    MdAtomicIncrement32(generation_);
    MdAtomicIncrement32(flush_request_);
}

void MdStereoRing::ApplyPendingFlush() noexcept {
    const uint32_t request = MdAtomicLoad32(flush_request_);
    if (request == flush_acked_) return;
    // Only the consumer stores read_frame_, so snapping it onto the producer cursor can
    // never push it past the producer.
    MdAtomicStore64(read_frame_, MdAtomicLoad64(write_frame_));
    MdAtomicStore32(low_watermark_, capacity_);
    flush_acked_ = request;
}

uint32_t MdStereoRing::AvailableRead() const noexcept {
    const uint64_t w = MdAtomicLoad64(write_frame_);
    const uint64_t r = MdAtomicLoad64(read_frame_);
    const uint64_t a = w >= r ? w - r : 0;
    return static_cast<uint32_t>(a > capacity_ ? capacity_ : a);
}
...
uint32_t MdStereoRing::Push(const float* s, uint32_t frames) noexcept {
    ...
    MdAtomicStore64(write_frame_, w + accepted);
    MdAtomicMax32(high_watermark_, used + accepted);
    MdAtomicMin32(low_watermark_, used + accepted);
    if (accepted < frames) MdAtomicAdd64(dropped_frames_, frames - accepted);
    return accepted;
}
...
uint32_t MdStereoRing::Pop(float* d, uint32_t frames, bool fill) noexcept {
    ...
    for (uint32_t f = 0; f < copied; ++f) {
        ...
        d[f * 2] = storage_[slot * 2];
        d[f * 2 + 1] = storage_[slot * 2 + 1];
    }
    MdAtomicStore64(read_frame_, r + copied);
    MdAtomicMin32(low_watermark_, a - copied);
    if (fill && copied < frames) {
        memset(d + copied * 2, 0, sizeof(float) * (frames - copied) * 2);
        MdAtomicAdd64(silent_frames_, frames - copied);
    }
    return copied;
}
...
uint32_t MdStereoRing::DiscardOldest(uint32_t frames) noexcept {
    ...
    MdAtomicStore64(read_frame_, r + discarded);
    MdAtomicAdd64(discarded_frames_, discarded);
    MdAtomicMin32(low_watermark_, a - discarded);
    return discarded;
}

MdRingStats MdStereoRing::Stats() const noexcept {
    MdRingStats s{};
    s.write_frames = MdAtomicLoad64(write_frame_);
    s.read_frames = MdAtomicLoad64(read_frame_);
    s.dropped_frames = MdAtomicLoad64(dropped_frames_);
    s.silent_frames = MdAtomicLoad64(silent_frames_);
    s.discarded_frames = MdAtomicLoad64(discarded_frames_);
    s.capacity_frames = capacity_;
    s.fill_frames = AvailableRead();
    s.high_watermark_frames = MdAtomicLoad32(high_watermark_);
    s.low_watermark_frames = MdAtomicLoad32(low_watermark_);
    s.generation = MdAtomicLoad32(generation_);
    return s;
}
