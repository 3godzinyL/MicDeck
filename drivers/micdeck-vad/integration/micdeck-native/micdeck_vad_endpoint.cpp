...
bool IsMicDeckVadEndpoint(IMMDevice* d,EDataFlow expected,std::wstring* container){
    ...
    // PKEY_Device_InstanceId on an endpoint is the MMDEVAPI endpoint id, never the
    // ROOT\MICDECKVAD hardware path, and PKEY_Device_FriendlyName is whatever the user
    // renamed the endpoint to. PKEY_DeviceInterface_FriendlyName carries the adapter name
    // straight from the INF, so it is the one signal that actually survives a rename.
    const auto adapter=prop(s.Get(),PKEY_DeviceInterface_FriendlyName);
    const auto cid=prop(s.Get(),PKEY_Device_ContainerId);
    const bool match=contains_ci(adapter,L"MICDECK")||
                     contains_ci(instance,L"MICDECKVAD")||
                     contains_ci(friendly,L"MICDECK");
