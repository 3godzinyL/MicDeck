...
bool WasapiCapture::active() const noexcept {
...
void WasapiCapture::run(
    ...
    while (!stopping_.load(std::memory_order_acquire) && !stream_failed) {
...
bool WasapiRender::active() const noexcept {
    return active_.load(std::memory_order_acquire);
}
...
void WasapiRender::run(std::wstring endpoint_id, FillCallback callback) {
    ...
    while (!stopping_.load(std::memory_order_acquire)) {
        ...
        result = render->ReleaseBuffer(available, 0);
        if (FAILED(result)) {
            break;
        }
    }
...
bool AudioEngine::healthy() const noexcept {
    return capture_.active() && render_.active();
}
