pub fn digest_hex(input:&[u8])->String{
 const H:[u32;8]=[0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,
 ...
 let bit_len=(input.len() as u64)*8;let mut p=input.to_vec();p.push(0x80);
 while p.len()%64!=56{p.push(0);}p.extend_from_slice(&bit_len.to_be_bytes());
 let mut state=H;
 for block in p.chunks_exact(64){
  let mut w=[0u32;64];for(i,b)in block.chunks_exact(4).enumerate(){
   w[i]=u32::from_be_bytes(b.try_into().unwrap());}
  for i in 16..64{let s0=w[i-15].rotate_right(7)^w[i-15].rotate_right(18)^(w[i-15]>>3);
   let s1=w[i-2].rotate_right(17)^w[i-2].rotate_right(19)^(w[i-2]>>10);
   w[i]=w[i-16].wrapping_add(s0).wrapping_add(w[i-7]).wrapping_add(s1);}
  let(mut a,mut b,mut c,mut d,mut e,mut f,mut g,mut h)=
   (state[0],state[1],state[2],state[3],state[4],state[5],state[6],state[7]);
  for i in 0..64{let s1=e.rotate_right(6)^e.rotate_right(11)^e.rotate_right(25);
   let ch=(e&f)^((!e)&g);let t1=h.wrapping_add(s1).wrapping_add(ch)
...
#[cfg(test)]mod tests{use super::*;#[test]fn vector(){
