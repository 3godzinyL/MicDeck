...
const state = {
  ...
  virtualAudio: {
    ...
    ready: false,
    backend: 'vbCable',
    backendLabel: 'VB-CABLE',
    installerAttempted: false,
    restartRequired: false,
    customDriverPackageReady: false,
    thirdParty: true,
    error: null,
    vendor: 'VB-Audio / VB-CABLE Pack45',
    vendorUrl: 'https://vb-audio.com/Cable/',
...
async function refreshState() {
  ...
  if (!state.microphoneNameDirty) {
    state.microphoneNameInput = virtualAudio.microphoneName
      || (virtualAudio.backend === 'micDeckVad' ? 'MicDeck Virtual Microphone' : 'MicDeck Virtual Mic');
  }
  render();
}
...
async function setVirtualAudioBackend(backend) {
  if (state.isSwitchingAudioBackend || backend === state.virtualAudio.backend) return;
  const previous = state.virtualAudio.backend;
  state.isSwitchingAudioBackend = true;
  state.virtualAudio.backend = backend;
  render();
  try {
    await invoke('set_virtual_audio_backend', { backend });
    await refreshState();
    showToast(t('toast.audioBackendChanged', {
      backend: state.virtualAudio.backendLabel || backend
    }), 'success');
  } catch (error) {
    state.virtualAudio.backend = previous;
    await refreshState().catch(() => {});
    showToast(error, 'error');
  } finally {
    state.isSwitchingAudioBackend = false;
    render();
  }
}
...
async function installVirtualAudioDriver() {
  ...
  try {
    ...
    await refreshState();
    showToast(t('toast.driverInstalled', {
      backend: state.virtualAudio.backendLabel || state.virtualAudio.backend
    }), 'success');
  } catch (error) {
...
function settingsView() {
  ...
  const isCustomDriver = state.virtualAudio.backend === 'micDeckVad';
  const customPackageMissing = isCustomDriver && !state.virtualAudio.customDriverPackageReady;
  const driverInstallDisabled = state.isInstallingDriver || customPackageMissing;
  const driverHelp = state.virtualAudio.error
    || (state.virtualAudio.restartRequired
      ? t('settings.driverInstalledRestart')
      : customPackageMissing
        ? t('settings.customPackageMissing')
        : isCustomDriver
          ? t('settings.installCustomDriverHelp')
          : t('settings.installDriverHelp'));
  ...
  return `
    ...
    <div class="settings-grid">
      <section class="surface settings-card driver-card">
        ...
        <div class="audio-backend-switch" role="group" aria-label="${t('settings.audioBackend')}">
          <button class="audio-backend-option ${state.virtualAudio.backend === 'vbCable' ? 'is-active' : ''}" data-audio-backend="vbCable" ${state.isSwitchingAudioBackend ? 'disabled' : ''}>
            <strong>VB-CABLE</strong>
            <small>${t('settings.backendCompatibility')}</small>
          </button>
          <button class="audio-backend-option ${isCustomDriver ? 'is-active' : ''}" data-audio-backend="micDeckVad" ${state.isSwitchingAudioBackend ? 'disabled' : ''}>
            <strong>MicDeck VAD</strong>
            <small>${t('settings.backendCustom')}</small>
          </button>
        </div>

        <p class="helper-text backend-description">${isCustomDriver
          ? t('settings.customBackendDescription')
          : t('settings.vbBackendDescription')}</p>

        ${state.virtualAudio.ready ? `
          <div class="device-route-card">
            <span class="round-icon">${icon('route')}</span>
            <div>
              <small>${t('settings.mixOutput')} · ${escapeHtml(state.virtualAudio.backendLabel || '')}</small>
        ...
        ` : `
          <div class="setup-callout">
            ...
            <div>
              ...
              <p>${escapeHtml(driverHelp)}</p>
            </div>
          </div>
          <button class="button button-accent full-button" id="install-driver-btn" ${driverInstallDisabled ? 'disabled' : ''}>
            ${state.isInstallingDriver
              ? `<span class="spinner"></span> ${t('common.installing')}`
              : `${icon('download')} ${isCustomDriver ? t('settings.installCustomDriver') : t('settings.installDriver')}`}
          </button>
        `}
        ...
        ${isCustomDriver ? `
          <div class="driver-architecture-card">
            <div class="driver-architecture-head">
              <strong>${t('settings.driverArchitecture')}</strong>
            ...
            <p>${t('settings.driverArchitectureDescription')}</p>
          </div>
        ` : ''}

        <div class="vendor-note">
          ${t('settings.deviceLayer')}: ${escapeHtml(state.virtualAudio.vendor)}
          ${state.virtualAudio.vendorUrl
            ? ` · <a href="${escapeHtml(state.virtualAudio.vendorUrl)}" target="_blank" rel="noreferrer">${state.virtualAudio.thirdParty ? t('settings.donationware') : t('settings.driverSource')}</a>`
            : ` · ${t('settings.driverSource')}`}
        </div>
      </section>
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-audio-backend]').forEach((button) => {
    button.addEventListener('click', () => setVirtualAudioBackend(button.dataset.audioBackend));
  });
