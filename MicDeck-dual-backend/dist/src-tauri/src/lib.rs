...
fn list_input_devices_impl() -> Result<Vec<DeviceDto>, String> {
    let host = cpal::default_host();
    let virtual_ids = virtual_audio::all_managed_capture_endpoints()
        .into_iter()
    ...
    for device in devices {
        ...
        let is_managed_virtual = virtual_ids
            ...
            || (fingerprint.contains("micdeck")
                && (fingerprint.contains("virtual audio")
                    || fingerprint.contains("virtual microphone")
                    || fingerprint.contains("driver input")));
...
fn sync_virtual_audio_devices(
    ...
    let render = resolve_managed_endpoint(
        virtual_audio::render_endpoints(app.virtual_audio_backend),
        app.virtual_render_device.as_deref(),
    );
    let capture = resolve_managed_endpoint(
        virtual_audio::capture_endpoints(app.virtual_audio_backend),
...
fn get_virtual_audio_status(
    ...
    let selected_backend = app.virtual_audio_backend;
    let bootstrap = bootstrap
        .lock()
        .map_err(|_| "Driver state lock error".to_string())?
        .clone();
    let bootstrap = if bootstrap.backend == Some(selected_backend) {
        bootstrap
    } else {
        virtual_audio::DriverBootstrap::default()
    };
    ...
    Ok(VirtualAudioStatusDto {
        ...
        custom_driver_package_ready: virtual_audio::custom_driver_package_ready(),
        third_party: selected_backend.is_third_party(),
        error: bootstrap.error,
        vendor: selected_backend.vendor(),
        vendor_url: selected_backend.vendor_url(),
...
fn set_virtual_audio_backend(
    ...
    if let Ok(mut driver_state) = bootstrap.lock() {
        *driver_state = virtual_audio::bootstrap_driver(backend);
    }
    ...
    if virtual_audio::driver_is_ready(backend) {
        start_native_runtime(&mut app, &mut runtime);
    } else {
        runtime.startup_error = Some(backend.missing_route_message().to_string());
    }
    Ok(())
}
...
fn configure_native_runtime(
    ...
    let backend = app.virtual_audio_backend;
    let render = render.ok_or_else(|| backend.missing_route_message().to_string())?;
    let capture = capture.ok_or_else(|| backend.missing_route_message().to_string())?;
        ...
        .setup(|app| {
            ...
            if let Some(window) = app.get_webview_window("main") {
                ...
                tauri::async_runtime::spawn_blocking(move || {
                    let selected_backend = app_handle
                        .state::<Mutex<AppState>>()
                        .lock()
                        .map(|state| state.virtual_audio_backend)
                        .unwrap_or_default();
                    let driver_status = virtual_audio::bootstrap_driver(selected_backend);
