//! Hardware integration probe for the real MicDeck -> VB-CABLE route.
//!
...
fn main() -> Result<(), String> {
    let render = virtual_audio::cable_render_endpoints()
        .into_iter()
        .next()
        .ok_or("No VB-CABLE render endpoint")?;
    let capture = virtual_audio::cable_capture_endpoints()
        .into_iter()
        .next()
        .ok_or("No VB-CABLE capture endpoint")?;
    ...
    if cable_capture_peak < 0.10 {
        return Err("CABLE Output received silence or an unusably weak signal".into());
    }
